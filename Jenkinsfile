            ...
            steps{
                script{
                    sh '''
                        ...
                        echo $(curl -X POST --head --write-out %{http_code} --silent --output /dev/null -d "firstName=$CUSTOMER_NAME&address=someaddress&cash=1.23" $LOAD_BALANCER/$SERVICE_PATH/seatCustomer)
